...
export default function Checkout(){
  ...
  const [pag,setPag]=useState('delivery') // delivery = pagar na entrega
  const [done,setDone]=useState(false)
  useEffect(()=>{ try{ const raw=localStorage.getItem('carrinho_v4'); if(raw) setCarrinho(JSON.parse(raw)) }catch(e){} },[])
  const total = carrinho.reduce((s,it)=>s+it.preco,0)
  const confirmar=(e)=>{ e.preventDefault(); setDone(true); localStorage.removeItem('carrinho_v4') }
  return (
    <div className='min-h-screen bg-gray-50'>
      ...
      <main className='max-w-3xl mx-auto px-4 py-8'>
        {done ? <div className='bg-white p-6 rounded-xl shadow text-center'><h2 className='text-2xl font-bold mb-2'>Pedido confirmado!</h2><p>Pagamento: {pag==='delivery'?'Pagar na entrega':'Pagamento online (simulado)'}</p></div> : (
